			   ...
			   struct byte_array *ikm, uint8_t *out)
{
	...
	PRINT_MSG("\n\033[1mStarting HKDF-Extract (HMAC-SHA-256)...\033[0m\n\n");
	if (salt->ptr == NULL || salt->len == 0) {
		PRINT_MSG("\033[1mSalt (default - empty string -> zero string):\033[0m\n");
		uint8_t zero_salt[32] = { 0 };
		PRINT_ARRAY("Salt", zero_salt, 32);
	} else {
		PRINT_ARRAY("\033[1mSalt\033[0m", salt->ptr, salt->len);
	}
	PRINT_ARRAY("\033[1mIKM (Input Keying Material):\033[0m", ikm->ptr, ikm->len);

	/*all currently prosed suites use hmac-sha256*/
	if (alg != SHA_256) {
		PRINT_MSG("\033[1mError: Unsupported hash algorithm for HKDF-Extract.\033[0m\n");
		return crypto_operation_not_implemented;
	}
#ifdef TINYCRYPT
	...
	TRY_EXPECT(psa_destroy_key(key_id), PSA_SUCCESS);
#endif
	PRINT_MSG("\n\033[1mFinished HKDF-Extract.\033[0m\n\n");
	return ok;
}
				   ...
				   uint8_t *shared_secret)
{
	...
	if (alg == P256) {
		...
		// PRINT_ARRAY("\033[1mPK Decompressed\033[0m", pk_decompressed,
