			  ...
			  const uint8_t *issuer, struct byte_array *root_pk)
{
	...
	for (uint16_t i = 0; i < cred_array->len; i++) {
		...
		PRINT_ARRAY("\033[1mcred_array[i].ca.ptr\033[0m", cred_array->ptr[i].ca.ptr,
		...
		if (0 == memcmp(subject_id->p, issuer, subject_id->len)) {
			...
			PRINT_ARRAY("\033[1mRoot PK of the CA\033[0m", root_pk->ptr,
				    root_pk->len);
			  ...
			  struct byte_array *pk, bool *verified)
{
	...
	PRINT_MSG("\n\033[1mStart parsing an ASN.1 certificate.\033[0m\n\n");
	...
	PRINT_ARRAY("\033[1mcert.serial\033[0m", m_cert.serial.p,
		    (uint32_t)m_cert.serial.len);
	PRINT_ARRAY("\033[1mcert.issuer_raw\033[0m", m_cert.issuer_raw.p,
	...
	PRINT_ARRAY("\033[1mcert issuer_id\033[0m", issuer_id->p, (uint32_t)issuer_id->len);
	...
	/* deserialize signature from ASN.1 to raw concatenation of (R, S) */
	{
		...
		mbedtls_asn1_traverse_sequence_of(&pp, pp + m_cert.sig.len, 0,
						  0, 0, 0, deser_sign_cb,
						  &deser_sign_ctx);
		PRINT_ARRAY("\033[1mCertificate signature\033[0m", sig.ptr, sig.len);
	}
	...
	/* export the public key from certificate */
	{
		...
		PRINT_ARRAY("\033[1mPK from cert\033[0m", pk->ptr, pk->len);
	}
