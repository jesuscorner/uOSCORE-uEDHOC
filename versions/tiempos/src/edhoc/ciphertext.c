				...
				struct byte_array *plaintext)
{
	...
	if (ctxt != CIPHERTEXT2) {
		/*Associated data*/
		PRINT_MSG("\n\033[1mEncoding Associated Data...\033[0m\n");
		TRY(associated_data_encode(th, &aad));
		PRINT_ARRAY("\033[1mAssociated Data (AAD)\033[0m", aad.ptr, aad.len);
		PRINT_MSG("\n");
	} else {
		PRINT_MSG("\033[1mCIPHERTEXT2: No Associated Data needed.\033[0m\n\n");
		tag.len = 0;
	}
