	   ...
	   struct byte_array *ead1)
{
    ...
    PRINTF("DEBUG: Parsed C_I choice: %d\\n", m.message_1_C_I_choice);
    if (m.message_1_C_I_choice == message_1_C_I_int_c) {
        PRINTF("DEBUG: Parsed C_I int value: %d (0x%x)\\n", m.message_1_C_I_int, m.message_1_C_I_int);
    } else {
        PRINTF("DEBUG: Parsed C_I bstr len: %zu\\n", m.message_1_C_I_bstr.len);
        // Ensure value is not NULL before printing if len > 0
        if (m.message_1_C_I_bstr.len > 0 && m.message_1_C_I_bstr.value != NULL) {
           PRINT_ARRAY("DEBUG: Parsed C_I bstr value", m.message_1_C_I_bstr.value, m.message_1_C_I_bstr.len);
        } else if (m.message_1_C_I_bstr.len > 0) {
           PRINTF("DEBUG: Parsed C_I bstr value is NULL!\\n");
        }
    }
		  ...
		  struct byte_array *c_i)
{
	...
	PRINT_MSG("\033[1mSent Message 2:  G_Y, Enc( C_R, MAC_2, EAD_2 )\033[0m\n\n");
	...
	enum err (*ead_process)(void *params, struct byte_array *ead13))
{
	...
	elapsed_ms = (double)(end - start) * 1000.0 / CLOCKS_PER_SEC;
	printf("\033[32m[EDHOC Responder Core] message_1 processing + message_2 generation time: %.3f ms\033[0m\n", elapsed_ms);
	printf("\033[32m[EDHOC Responder Core] message_2 size: %u bytes\033[0m\n", rc.msg.len);
	...
	elapsed_ms = (double)(end - start) * 1000.0 / CLOCKS_PER_SEC;
	if (rc.ead.len > 0) {
		printf("\033[32m[EDHOC Responder Core] EAD_1 processing time: %.3f ms\033[0m\n", elapsed_ms);
	}
	...
	elapsed_ms = (double)(end - start) * 1000.0 / CLOCKS_PER_SEC;
	printf("\033[32m[EDHOC Responder Core] message_3 processing time: %.3f ms\033[0m\n", elapsed_ms);
	PRINT_MSG("\n---------------------------------------------\n\n");
	...
	elapsed_ms = (double)(end - start) * 1000.0 / CLOCKS_PER_SEC;
	if (rc.ead.len > 0) {
		printf("\033[32m[EDHOC Responder Core] EAD_3 processing time: %.3f ms\033[0m\n", elapsed_ms);
	}
	...
	elapsed_ms = (double)(end - start) * 1000.0 / CLOCKS_PER_SEC;
	printf("\033[32m[EDHOC Responder Core] message_4 generation time: %.3f ms\033[0m\n", elapsed_ms);
	printf("\033[32m[EDHOC Responder Core] message_4 size: %u bytes\033[0m\n", rc.msg.len);
	PRINT_MSG("\n---------------------------------------------\n\n");
	TRY(tx(c->sock, &rc.msg));
#endif // MESSAGE_4
	return ok;
