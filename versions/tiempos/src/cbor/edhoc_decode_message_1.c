		...
		zcbor_state_t *state, struct message_1 *result)
{
	...
	&& ((*result).message_1_ID_CRED_PSK_present = ((zcbor_bstr_decode(state, (&(*result).message_1_ID_CRED_PSK)))), 1)
		...
		size_t *payload_len_out)
{
	...
	return zcbor_entry_function(payload, payload_len, (void *)result, payload_len_out, states,
		(zcbor_decoder_t *)decode_message_1, sizeof(states) / sizeof(zcbor_state_t), 6);
}
