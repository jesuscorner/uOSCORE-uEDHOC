...
static bool decode_ptxt2sig(zcbor_state_t *state, struct ptxt2sig *result);
...
static bool decode_ptxt2sig(
		zcbor_state_t *state, struct ptxt2sig *result)
{
	zcbor_log("%s\r\n", __func__);
	bool int_res;

	bool tmp_result = (((((zcbor_union_start_code(state) && (int_res = ((((zcbor_int32_decode(state, (&(*result).ptxt2sig_C_R_int)))) && (((*result).ptxt2sig_C_R_choice = ptxt2sig_C_R_int_c), true))
	|| (((zcbor_bstr_decode(state, (&(*result).ptxt2sig_C_R_bstr)))) && (((*result).ptxt2sig_C_R_choice = ptxt2sig_C_R_bstr_c), true))), zcbor_union_end_code(state), int_res)))
	&& ((zcbor_union_start_code(state) && (int_res = ((((decode_map2(state, (&(*result).ptxt2sig_ID_CRED_X_map2_m)))) && (((*result).ptxt2sig_ID_CRED_X_choice = ptxt2sig_ID_CRED_X_map2_m_c), true))
	|| (zcbor_union_elem_code(state) && (((zcbor_bstr_decode(state, (&(*result).ptxt2sig_ID_CRED_X_bstr)))) && (((*result).ptxt2sig_ID_CRED_X_choice = ptxt2sig_ID_CRED_X_bstr_c), true)))
	|| (((zcbor_int32_decode(state, (&(*result).ptxt2sig_ID_CRED_X_int)))) && (((*result).ptxt2sig_ID_CRED_X_choice = ptxt2sig_ID_CRED_X_int_c), true))), zcbor_union_end_code(state), int_res)))
	&& ((zcbor_bstr_decode(state, (&(*result).ptxt2sig_SGN_or_MAC_2))))
	&& ((*result).ptxt2sig_EAD_2_present = ((zcbor_bstr_decode(state, (&(*result).ptxt2sig_EAD_2)))), 1))));
...
int cbor_decode_ptxt2sig(
		const uint8_t *payload, size_t payload_len,
		struct ptxt2sig *result,
		size_t *payload_len_out)
{
	zcbor_state_t states[6];

	return zcbor_entry_function(payload, payload_len, (void *)result, payload_len_out, states,
		(zcbor_decoder_t *)decode_ptxt2sig, sizeof(states) / sizeof(zcbor_state_t), 4);
}
