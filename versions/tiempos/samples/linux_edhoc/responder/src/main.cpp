...
#define ORIG
	...
	while (1) {
		...
		if (first_message_received) { // Ensure rx was actually called at least once
			...
			printf("\033[32m[EDHOC Responder] Total exchange time (from msg1 reception): %.3f ms\033[0m\n", total_elapsed_ms); // Print total time for this exchange in green
		} else {
			printf("\033[32m[EDHOC Responder] Error: First message not received, cannot calculate total time.\033[0m\n");
		}
