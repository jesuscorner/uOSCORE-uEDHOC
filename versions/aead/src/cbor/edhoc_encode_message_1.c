		...
		zcbor_state_t *state, const struct message_1 *input)
{
	...
	: false)))
	&& (!(*input).message_1_ID_CRED_PSK_present || zcbor_bstr_encode(state, (&(*input).message_1_ID_CRED_PSK)))
		...
		size_t *payload_len_out)
{
	...
	return zcbor_entry_function(payload, payload_len, (void *)input, payload_len_out, states,
		(zcbor_decoder_t *)encode_message_1, sizeof(states) / sizeof(zcbor_state_t), 6);
}
