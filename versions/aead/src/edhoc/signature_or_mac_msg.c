		    ...
		    struct suite *suite, struct byte_array *mac)
{
	...
	PRINT_ARRAY("\033[1mMAC context\033[0m", context_mac.ptr, context_mac.len);
				     ...
				     struct byte_array *out)
{
	...
	PRINT_ARRAY("\033[1mCOSE_Sign1 object to be signed\033[0m", out->ptr, out->len);
	return ok;
}
		 ...
		 enum info_label mac_label, struct byte_array *signature_or_mac)
{
	if (op == GENERATE) {
		...
		if (static_dh) {
			...
			PRINT_MSG(
				"\n\033[1mSignature or MAC verification successful!\033[0m\n\n");
		}
	}
	return ok;
}
