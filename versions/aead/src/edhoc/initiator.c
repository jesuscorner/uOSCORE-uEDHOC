	...
	enum err (*ead_process)(void *params, struct byte_array *ead24))
{
	...
	start = clock();
	TRY(msg1_gen(c, &rc));
	// Calculate elapsed time for message 1 generation
	end = clock();
	elapsed_ms = (double)(end - start) * 1000.0 / CLOCKS_PER_SEC;
	printf("\n[EDHOC Initiator Core] message_1 size: \033[32m%u bytes\033[0m, generation time: \033[32m%.3f ms\033[0m\n", rc.msg.len, elapsed_ms);
	PRINT_MSG("\n---------------------------------------------\n\n");
	...
	elapsed_ms = (double)(end - start) * 1000.0 / CLOCKS_PER_SEC;
	printf("\n[EDHOC Initiator Core] message_2 processing + message_3 generation time: \033[32m%.3f ms\033[0m\n", elapsed_ms);
	printf("[EDHOC Initiator Core] message_3 size: \033[32m%u bytes\033[0m\n", rc.msg.len);
	...
	elapsed_ms = (double)(end - start) * 1000.0 / CLOCKS_PER_SEC;
	if (rc.ead.len > 0) {
		printf("[EDHOC Initiator Core] EAD_2 processing time: \033[32m%.3f ms\033[0m\n", elapsed_ms);
	}
	PRINT_MSG("\n---------------------------------------------\n\n");
...
/*receive message 4*/
	...
	elapsed_ms = (double)(end - start) * 1000.0 / CLOCKS_PER_SEC;
	printf("[EDHOC Initiator Core] message_4 processing time: \033[32m%.3f ms\033[0m\n", elapsed_ms);
	PRINT_MSG("\n---------------------------------------------\n\n");

	// EAD_4 processing timing (optional)
	start = clock();
	TRY(ead_process(c->params_ead_process, &rc.ead));
	end = clock();
	elapsed_ms = (double)(end - start) * 1000.0 / CLOCKS_PER_SEC;
	if (rc.ead.len > 0) {
		printf("[EDHOC Initiator Core] EAD_4 processing time: \033[32m%.3f ms\033[0m\n", elapsed_ms);
	}
#endif // MESSAGE_4
	return ok;
}
