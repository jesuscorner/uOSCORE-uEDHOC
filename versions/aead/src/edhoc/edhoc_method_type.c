...
void authentication_type_get(enum method_type m, volatile bool *static_dh_i, volatile bool *static_dh_r, volatile bool *is_psk)
{
	switch (m) {
	...
	case INITIATOR_SDHK_RESPONDER_SDHK:
		...
		*is_psk=false;
		break;
	case INITIATOR_PSK_RESPONDER_PSK:
		*static_dh_i = false;
		*static_dh_r = false;
		*is_psk=true;
		break;
